...
describe('api', () => {
  ...
  it('sends the session cookie only to its own origin', async () => {
    const calls = mockFetch({ 'GET /api/me': { body: { name: 'bob', csrf: 'x' } }, 'POST /api/logout': { body: {} } });
    await get('/api/me');
    await post('/api/logout');
    // A relative URL plus same-origin credentials: the cookie can never be
    // attached to a request for another origin.
    expect(calls.map((c) => c.url)).toEqual(['/api/me', '/api/logout']);
    expect(calls.map((c) => c.credentials)).toEqual(['same-origin', 'same-origin']);
  });
